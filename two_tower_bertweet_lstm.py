...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/two_tower_bertweet_lstm/{model_name}_{test_size}_{timestamp}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

#%% Dataset Class for Two-Tower Model
...
#%% Two-Tower BERTweet-LSTM Model
class TwoTowerBERTweetClassifier(torch.nn.Module):
    def __init__(self, bertweet_model_name="models/bertweet-base", num_classes=2):
        super().__init__()
        
        # Tweet Tower
        self.tweet_bertweet = AutoModel.from_pretrained(bertweet_model_name)
        self.tweet_lstm = torch.nn.LSTM(
            input_size=768,  # BERTweet hidden size
            hidden_size=64,
            num_layers=2,
            bidirectional=True,
            dropout=0.1,
        ...
        self.note_bertweet = AutoModel.from_pretrained(bertweet_model_name)
    ...
    def forward(self, tweet_input_ids, tweet_attention_mask, note_input_ids, note_attention_mask):
        # Process tweet tower
        tweet_output = self.tweet_bertweet(
        ...
        note_output = self.note_bertweet(
...
if __name__ == '__main__':
    ...
    logger.info("Running Two-Tower BERTweet model training...")
    
    # Configuration
    MAX_LEN = 128
    BATCH_SIZE = 16  # 32 for 12GB VRAM, 16 for 6GB VRAM
    EPOCHS = 100
    LEARNING_RATE = 2e-5
    BERTWEET_MODEL = "models/bertweet-base"
    ...
    model = TwoTowerBERTweetClassifier(bertweet_model_name=BERTWEET_MODEL, num_classes=len(np.unique(y_processed)))
    ...
    logger.info(f"Using BERTweet model: {BERTWEET_MODEL}")
    ...
    X_tweet_val, X_tweet_test, X_notes_val, X_notes_test, y_val, y_test = train_test_split(
        X_tweet_temp, X_notes_temp, y_temp, test_size=0.6, random_state=42  # This gives ~20% test, 15% validation
    )
    ...
    # Using different learning rates for pretrained model and new layers
    no_decay = ['bias', 'LayerNorm.weight']
    optimizer_grouped_parameters = [
        # BERTweet parameters with weight decay
        {
            'params': [p for n, p in model.named_parameters() 
                      if ('bertweet' in n) and not any(nd in n for nd in no_decay)],
            'weight_decay': 0.01,
            'lr': LEARNING_RATE
        },
        # BERTweet parameters without weight decay
        {
            'params': [p for n, p in model.named_parameters() 
                      if ('bertweet' in n) and any(nd in n for nd in no_decay)],
            'weight_decay': 0.0,
            'lr': LEARNING_RATE
        },
        # Non-BERTweet parameters (LSTM, fusion, classifier) with higher learning rate
        {
            'params': [p for n, p in model.named_parameters() if not ('bertweet' in n)],
            'weight_decay': 0.01,
            'lr': LEARNING_RATE * 10
        }
    ]
    ...
    with open(f'metrics/two_tower_bertweet_lstm/{model_name}_training_metrics_{timestamp_day}.pkl', 'wb') as f:
        pickle.dump(training_metrics, f)
    logger.info(f"Training metrics saved to metrics/two_tower_bertweet_lstm/{model_name}_training_metrics_{timestamp_day}.pkl")
    ...
    # best_model_path = f'models/two_tower_bertweet_final_20250317.pth'
    ...
    with open(f'metrics/two_tower_bertweet_lstm/{model_name}_test_metrics_{timestamp_day}.pkl', 'wb') as f:
        pickle.dump(test_metrics, f)
    logger.info(f"Test metrics saved to metrics/two_tower_bertweet_lstm/{model_name}_test_metrics_{timestamp_day}.pkl")
  
    # Save final model regardless of performance
    final_model_path = f'models/two_tower_bertweet_final_{timestamp_day}.pth'
    if isinstance(model, DataParallel):
